...
import { Clock, FileText, CheckCircle, User, Calendar, ChevronRight, Eye, Upload, Mail } from "lucide-react";
...
export default function AssignmentCard({ assignment, studentId }: AssignmentCardProps) {
  ...
  const teacherInitials = assignment.teacher?.name ? getAvatarInitials(assignment.teacher.name) : "T";
  const avatarColor = assignment.teacher?.id ? getAvatarColor(assignment.teacher.id) : 'bg-gray-500 text-white';
    ...
    <div className="group p-6 hover:bg-slate-50/50 transition-colors border-b">
      <div className="flex flex-col lg:flex-row lg:items-start gap-6">
          ...
          <div className="flex items-start gap-4">
            ...
            <div className="flex flex-col items-center gap-3">
              <Avatar className="h-12 w-12 border-2 border-white shadow-sm">
                <AvatarImage src={assignment.teacher?.image} alt={assignment.teacher?.name} />
              ...
              <div className={`p-2 rounded-full ${
                !mySubmission ? 'bg-red-100 text-red-600' :
                mySubmission?.grade === null ? 'bg-amber-100 text-amber-600' :
                'bg-green-100 text-green-600'
              }`}>
                {!mySubmission ? <Clock className="w-4 h-4" /> :
                 mySubmission?.grade === null ? <FileText className="w-4 h-4" /> :
            ...
            <div className="flex-1">
              <div className="flex flex-wrap items-center gap-3 mb-2">
                ...
                <Badge variant={
                  !mySubmission ? "destructive" :
                  mySubmission?.grade === null ? "outline" :
                  "default"
              ...
              <div className="flex flex-wrap items-center gap-4 text-sm text-slate-500">
                <div className="flex items-center gap-2">
                  <div className="flex items-center gap-2">
                    ...
                    <span className="font-medium text-slate-700">{assignment.teacher?.name || "Instructor"}</span>
                  </div>
        ...
        <div className="lg:w-48 flex flex-col gap-3">
          {!mySubmission ? (
            <Button asChild className="gap-2 bg-gradient-to-r from-blue-500 to-blue-600 hover:from-blue-600 hover:to-blue-700">
